...
export function StoreListingCard({ listing, isMember, currentUserId }: Props) {
  ...
  return (
    <>
      <Link href={`/store/${listing.id}`} className={styles.card} onClick={handleClick}>
        {/* Corner accent */}
        ...
        <div className={styles.imageArea}>
          ...
          {/* Guest overlay — shown for all unauthenticated users */}
          {isGuest && (
          ...
          {/* Lock overlay for members-only (authenticated non-member) */}
          {!isGuest && isLocked && (
            <div className={styles.lockOverlay}>
              <svg width="22" height="22" viewBox="0 0 22 22" fill="none" aria-hidden="true">
                <rect x="3" y="9" width="16" height="11" rx="2" stroke="#898882" strokeWidth="1.4" />
          ...
          {/* Sold overlay */}
          {listing.is_sold && !isLocked && !isGuest && (
        ...
        <div className={styles.content}>
          ...
          <div className={styles.footer}>
            {isGuest ? (
              <span style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: 11, color: '#C8FF00', letterSpacing: '0.06em', opacity: 0.8 }}>
                JOIN TO VIEW →
              </span>
            ) : isLocked ? (
